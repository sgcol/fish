...
function afterUserIn(err, pack, ws, dbuser) {
    ...
    if (dbuser) {
        if (!dbuser.__created) {
            ...
            ws.sendp({ user: { showId: dbuser.showId, isAdmin: dbuser.isAdmin, savedMoney: dbuser.savedMoney, vipLv: dbuser.vipLv, firstBuy: dbuser.firstBuy, nickname: dbuser.nickname } });
        } else {
            dbuser.pwd = pack.pwd;
            g_db.p.users.find({ _id: 'showId' }).limit(1).toArray(function(err, result) {
                ...
                dbuser.bank = dbuser.bank;
                dbuser.province = pack.province;
                dbuser.city = pack.city;
                dbuser.coins = dbuser.coins;
                dbuser.reward = dbuser.reward; //悬赏
                ws.sendp({ user: { showId: dbuser.showId, isAdmin: dbuser.isAdmin, bank: dbuser.bank, savedMoney: dbuser.savedMoney, vipLv: dbuser.vipLv, firstBuy: dbuser.countDown } });
    ...
    if (!oldUser) {
        ...
        dbuser.nickname = pack.nickname || dbuser.nickname || pack.id;
    ...
    if (dbuser.isAdmin == true) {
        ws.sendp({ c: 'showview', v: 'manager', user: dbuser, seq: 1 });
    } else {
...
module.exports = function msgHandler(db, createDbJson, wss) {
    ...
    wss.on('connection', function connection(ws) {
        ...
        ws.on('message', function(data) {
            ...
            .on('close', function() {
                if (ws.user) {
                    ws.user.dbuser.lastOfflineTime = new Date();
                    ws.user.dbuser.rewardType = 0;
                    ws.user.dbuser.onlineTime += parseInt(ws.user.dbuser.lastOfflineTime - ws.user.dbuser.loginTime) / (1000 * 60);
