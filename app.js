...
var getDB = require('./server/db.js'),
...
var jrpc = new(require('jrpc-client'))()
    ...
    .on('connect', function regServer() {
        ...
        jrpc.call('serverinfo', {
            ...
            $: {
                upsert: {
                    c: { platformID: PLATFORM_ID, serverID: SERVER_ID },
                    set: {
                        clusterID: PLATFORM_ID + '_' + SERVER_ID,
                        data: {
                            ip: ip,
                            port: port,
                            gm: '/gm',
...
app.use('/pf/:interface', function(req, res, next) {
    ...
    if (external_pf[req.pf]) return external_pf[req.pf].call(null, req, res, function(err) {
        ...
        res.status(404).end('no such function ' + req.url, err);
    });
...
var wxApp = (function() {
    ...
    function saveT(openid, t, callback) {
        wechatTokens[openid] = t;
        callback();
    }
    var apps = [
...
app.all('/CORS', function(req, res) {
    ...
    if (req.query.s) {
        delete req.query.s;
        delete req.query.u;
        url += '&' + strObj(req.query);
        console.log(url)
    }
    request(url).on('reponse', function(res) {
        console.log(res);
...
getDB(function(err, db, easym) {
    ...
    app.all('/pay', httpf({ orderid: 'string', money: 'number', callback: true }, function(orderid, money, callback) {
        console.log('who pa who')
        if (argv.debugout) return callback('only availble in debugout mode');
        confirmOrder(orderid, money, '127.0.0.1', callback);
    ...
    confirmOrder = function(orderid, money, userip, extra, callback) {
        ...
        debugout('confirmOrder', orderid, money, userip, extra, callback);
        var self = this;
        try {
            var key = { _id: ObjectID(orderid) };
            db.bills.find(key).limit(1).next(function(err, order) {
                ...
                User.fromID(order.user, function(err, user) {
                    if (err) return callback(err);
                    if (user.dbuser.bought) {
                        if (user.dbuser.bought.indexOf(order.pack.name) != -1) {
                            console.log('only once')
                            return callback('只能购买一次');
                        } else if (order.pack.name == 'o77Coin') {
                            return callback('小礼包购买后，不能再买超值礼包');
                        }
                    }
                    if (user.dbuser.tehui) {
                        if (user.dbuser.tehui.indexOf(order.pack.name) != -1) {
                            return callback('每天只能购买一次');
                        }
                    }
                    user.addPackage(order.pack);
